...
export default function Result() {
  ...
  return (
    <Layout>
      <div className="flex h-full w-full flex-col min-h-screen items-center pt-16 text-2xl" style={bookPageStyle}>
      <div className="space-x-8" style={{ alignSelf: 'flex-end', marginRight: '0rem' }}>
        ...
        {output.map((item, index) => (
          <div key={index} style={textStyle}>
            {item.speaker === "ai" ? (
              <div className="text-left text-2xl font-bold mt-4 mb-4">
                {item.text}
              </div>
            ) : (
              splitTextIntoChunks(item.text).map((chunk, chunkIndex) => (
                <div key={chunkIndex} className="text-center text-xl mb-8 mt-0">
                  {chunk}
                </div>
              ))
            )}
